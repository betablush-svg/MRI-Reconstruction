...
class RefineGANGenerator(nn.Module):
    ...
    def __init__(self, stages=3):
        ...
        self.stages = nn.ModuleList(
            [RefineBlock() for _ in range(stages)]
        )
...
class RefineGAN(nn.Module):
    ...
    def __init__(self, stages=3):
        ...
        self.generator = RefineGANGenerator(stages)
...
def train_model(train_loader, val_loader, epochs=10, lr=1e-4):
    ...
    G = RefineGAN().to(device)
    D = RefineGANDiscriminator().to(device)
    ...
    os.makedirs("/datasets/tir-datasets-mri-recon/results/checkpoints/refinegan", exist_ok=True)
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1} Training"):
            ...
            k = ensure_4d(batch["kspace_under"].to(device).float())
        ...
        if val_psnr > best_psnr:
            ...
            torch.save(
                G.state_dict(),
                "/datasets/tir-datasets-mri-recon/results/checkpoints/refinegan/best_generator.pt"
            )
        ...
        if((epoch+1)%10==0):
            torch.save(
                G.state_dict(),
                f"/datasets/tir-datasets-mri-recon/results/checkpoints/refinegan/epoch_{epoch+1}.pt"
            )
    ...
    # --------------------------
    with open("/datasets/tir-datasets-mri-recon/results/checkpoints/refinegan/history.json", "w") as f:
        json.dump(history, f)
