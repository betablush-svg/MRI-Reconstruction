...
def compute_grappa(kspace,mask, accel=4):
    ...
    # ----- ACS region -----
    acs_size = 24 if accel==4 else 32
    cx, cy = H//2, W//2
    acs = kspace * (mask == 1)

    # ----- GRAPPA -----
    kspace_grappa = grappa(kspace_under, acs, kernel_size=(5,5))
    kspace_grappa = torch.tensor(kspace_grappa, dtype=torch.complex64)

    coil_imgs = ifft2c(torch.tensor(kspace_grappa))
    img_grappa = rss_combine(coil_imgs)
    img_grappa /= img_grappa.max()

    return img_grappa

def grappa_slice_recon(full_kspace, accel=4):

    kspace = to_complex_kspace(full_kspace)   # (coils,H,W)
    coils,H,W = kspace.shape

    # ----- Ground truth -----
    coil_imgs = ifft2c(torch.tensor(kspace))
    ...
    return img_grappa, img_gt, recon_time
...
def evaluate_dataset(folder, accel=4):
    ...
    for file in tqdm(files):
        ...
        for slice_k in kspace_vol:
            pred, gt, t = grappa_slice_recon(slice_k, accel)
