...
class VarNetBlock(nn.Module):
    def __init__(self):
        ...
        self.lambda_dc = nn.Parameter(torch.tensor(0.5))
    ...
    def forward(self, x, k_under, mask):
        ...
        grad = torch.fft.ifft2(
            mask * (x_k - k_under),
            norm='ortho'
        ).real

        dc_update = self.lambda_dc * grad

        # ---- Learned Regularizer ----
        reg_update = self.reg(x)

        # ---- Update Step ----
        x = x - dc_update - reg_update

        return x
    
class VarNet(nn.Module):
    def __init__(self, cascades=8):
        super().__init__()

        self.cascades = nn.ModuleList([
            VarNetBlock() for _ in range(cascades)
        ])

    def forward(self, x, k_under, mask):

        for cascade in self.cascades:
            x = cascade(x, k_under, mask)
...
def train_model(train_loader, val_loader, epochs=10, lr=1e-4):
    ...
    model = VarNet(cascades=8).to(device)

    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    criterion = nn.MSELoss()

    os.makedirs("/datasets/tir-datasets-mri-recon/results/checkpoints/varnet", exist_ok=True)
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(
                model.state_dict(),
                "/datasets/tir-datasets-mri-recon/results/checkpoints/varnet/best_model.pt"
            )
    ...
    # -------------------------
    with open("/datasets/tir-datasets-mri-recon/results/checkpoints/varnet/history.json", "w") as f:
        json.dump(history, f)
