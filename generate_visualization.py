...
'''for name, model in models.items():
        ...
        with torch.no_grad():
                for sample in test_loader:
                        ...
                        input_data = sample['img_zf']      # undersampled input
                        target = sample['target']       # fully sampled ground truth
                        mask = sample['mask']
                        k_under = sample['kspace_under']
                        k_space = sample['kspace_2ch']

                        if name=='GT':
                            all_recons.append(target.squeeze().cpu().numpy())
                        elif name=="KIKI":
                            out = model(k_space)
                            recons = to_magnitude(out).squeeze().cpu().numpy()
                            all_recons.append(recons)
                        else:
                            out = model(input_data, k_under, mask)
                            recons = to_magnitude(out).squeeze().cpu().numpy()
                            all_recons.append(recons)
...
save_root = "Images\\file1000021\\saved_slices"
...
for name, model in models.items():
    ...
    with torch.no_grad():
        ...
        for sample in test_loader:
            ...
            if name == 'GT':
                ...
                out = model(k_space)
                recon = to_magnitude(out).squeeze().cpu().numpy()
    ...
    # Save only selected slices
    for idx in indices:

        slice_img = imgs[idx]

        # Normalize for proper PNG saving
        slice_img = (
